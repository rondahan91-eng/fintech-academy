...
function doOnboard(req) {
  ...
  // ⛔ **לפני הכתיבה לגיליון, ולא אחריה.** השרשור רץ קודם אחרי
  //    הכתיבה, ואז מה שהתלמיד ראה היה שונה ממה שנשמר — ואם
  //    התוספת היא שאלת מדידה, הגיליון לא ידע שהיא נשאלה,
  //    ‏askedCount לא היה סופר אותה, ואלעד היה שואל שוב.
  //    ‏done מחושב ראשון, אחרת מוסיפים שאלה להודעה האחרונה.
  out.done = doneFrom(turns.concat([{ role: 'elad', text: out.say }]), out.done);
